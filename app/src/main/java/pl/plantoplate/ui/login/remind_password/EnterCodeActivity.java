...
public class EnterCodeActivity extends AppCompatActivity {
    ...
    public void validateConfirmCode(View view) {
        String enteredCode = enterCodeTextInput.getText() != null ? enterCodeTextInput.getText().toString().trim(): "";
        String correctCode = prefs.getString("code", "");
        if (correctCode.equals(enteredCode)){
            startActivity(new Intent(this, ChangePasswordActivity.class));
        }
        else {
            Snackbar.make(view, "Wprowadzony kod jest niepoprawny", BaseTransientBottomBar.LENGTH_LONG).show();
        }
    }
    ...
    public void resendCode(View view) {
        ...
        Disposable disposable = repository.getEmailConfirmCode(email, "reset")
                .subscribe(
                        ...
                        error -> Snackbar.make(view, Objects.requireNonNull(error.getMessage()), BaseTransientBottomBar.LENGTH_LONG).show()
                );
